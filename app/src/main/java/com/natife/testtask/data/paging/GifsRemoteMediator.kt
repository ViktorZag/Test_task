...
class GifsRemoteMediator (
   ...
   private val database: AppDatabase
    ...
    override suspend fun load(
        ...
        val response = api.getTrendingGifs(limit = 20, offset = 20 * currentPage)
        ...
        database.withTransaction {
            ...
            val keys = response.data.map { gifDto ->
                KeysEntity(
                    id = gifDto.id,
                    prevPage = prevPage,
                    nextPage = nextPage
            ...
            database.keysDao.addAllGifKeys(remoteKeys = keys)
            database.gifsDao.addGifs(gifs = response.toGifEntityList())
        }
